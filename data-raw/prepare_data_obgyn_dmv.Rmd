---
title: "prepare data - catchment_scores - obgyn"
author: "--"
date: "2/4/2022"
...
# load data provider
```{r}
provider <- read.csv("obgyn.dmv.geo.csv", row.names = 1)
```
# fix data
```{r}
...
# blocks_VA <- st_as_sf(blocks(state="VA", year=2014))
# blocks_MD <- st_as_sf(blocks(state="MD", year=2014))
# blocks_DC <- st_as_sf(blocks(state="DC", year=2014))
...
num_providers <- provider %>% group_by(GEOID) %>% summarise(doctors=sum(doctors))
...
population <- data_combined %>% select(GEOID, female_over_14, doctors, near_10_mean, near_10_median)
